...
$loja_imagem = new ConteudosAssetsModel();
...
$loja_imagem->selectOne('*', $loja_imagem->getPrimaryKeyName() .'='. $cod);
$row = $loja_imagem->getResults();

if ($_POST['acao'] == 'cortar') {

  $filesys->excluir(SYS_CONTEUDO .'uploads/'. $row['imagem'. $numero]);
  
  $src       = SYS_CONTEUDO .'uploads/'. $row['imagem'. $numero .'_original'];
  ...
  $loja_imagem->update(array('imagem'. $numero => $novo_nome), $loja_imagem->getPrimaryKeyName() .'='. $cod);
