 ...
 * Itaucripto Class
 *
 * Essa classe permite utilizar as funÃ§Ãµes de criptografia do ItaÃº Shopline
...
class Itaucripto 
{
	...
	public function geraDados($paramString1, $paramString2, $paramString3, $paramString4, $paramString5, $paramString6, $paramString7, $paramString8, $paramString9, $paramString10, $paramString11, $paramString12, $paramString13, $paramString14, $paramString15, $paramString16, $paramString17, $paramString18)
	{
		...
		if (strlen($paramString1) != $this->TAM_COD_EMP) {
			return "Erro: tamanho do codigo da empresa diferente de 26 posiÃ§Ãµes.";
		}

		if (strlen($paramString5) != $this->TAM_CHAVE) {
			return "Erro: tamanho da chave da chave diferente de 16 posiÃ§Ãµes.";
		}

		if ((strlen($paramString2) < 1) || (strlen($paramString2) > 8)) {
			return "Erro: nÃºmero do pedido invÃ¡lido.";
		}

		if (is_numeric($paramString2)) {
			$paramString2 = $this->PreencheZero($paramString2, 8);
		} else {
			return "Erro: numero do pedido nÃ£o Ã© numÃ©rico.";
		}

		if ((strlen($paramString3) < 1) || (strlen($paramString3) > 11)) {
			return "Erro: valor da compra invÃ¡lido.";
		}
		...
		if ($i !== FALSE) {
			...
			if (!is_numeric($str3)) {
				return "Erro: valor decimal nÃ£o Ã© numÃ©rico.";
			}

			if (strlen($str3) != 2) {
				return "Erro: valor decimal da compra deve possuir 2 posiÃ§Ãµes apÃ³s a virgula.";
			}

			$paramString3 = substr($paramString3, 0, strlen($paramString3) - 3) . $str3;
		} else {
			if (!is_numeric($paramString3)) {
				return "Erro: valor da compra nÃ£o Ã© numÃ©rico.";
			}

			if (strlen($paramString3) > 8) {
				return "Erro: valor da compra deve possuir no mÃ¡ximo 8 posiÃ§Ãµes antes da virgula.";
			}
		...
		if (($paramString7 != "02") && ($paramString7 != "01") && ($paramString7 != "")) {
			return "Erro: cÃ³digo de inscriÃ§Ã£o invÃ¡lido.";
		}

		if (($paramString8 != "") && (!is_numeric($paramString8)) && (strlen($paramString8) > 14)) {
			return "Erro: nÃºmero de inscriÃ§Ã£o invÃ¡lido.";
		}
		...
		if (($paramString14 != "") && ((!is_numeric($paramString14)) || (strlen($paramString14) != 8))) {
			return "Erro: data de vencimento invÃ¡lida.";
		}

		if (strlen($paramString16) > 60) {
			return "Erro: observaÃ§Ã£o adicional 1 invÃ¡lida.";
		}

		if (strlen($paramString17) > 60) {
			return "Erro: observaÃ§Ã£o adicional 2 invÃ¡lida.";
		}

		if (strlen($paramString18) > 60) {
			return "Erro: observaÃ§Ã£o adicional 3 invÃ¡lida.";
		}
	...
	public function geraCripto($paramString1, $paramString2, $paramString3) 
	{
		...
		if ($paramString2 == "") {
			return "Erro: cÃ³digo do sacado invÃ¡lido.";
		}
