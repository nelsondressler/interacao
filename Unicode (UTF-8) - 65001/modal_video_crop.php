...
$video->selectOne('*', $video->getPrimaryKeyName() .'='. $cod);
$row = $video->getResults();

if ($_POST['acao'] == 'cortar') {

  $filesys->excluir(SYS_CONTEUDO .'videos/'. $row['imagem']);
  
  $src       = SYS_CONTEUDO .'videos/'. $row['imagem_original'];
  ...
  $image->save(SYS_CONTEUDO .'videos/'. $novo_nome);
  $video->update(array('imagem' => $novo_nome), $video->getPrimaryKeyName() .'='. $cod);
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <img src="<?php echo SYS_SITE_CONTEUDO .'videos/'. $novo_nome ?>" border="0" style="max-height: 400px; max-width: 400px"/>
      <br><br>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td height="425" align="center"><img src="<?php echo SYS_SITE_CONTEUDO .'videos/'. $row['imagem_original'] ?>" id="cropbox" /></td>
  </tr>
</table>
