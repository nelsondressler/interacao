...
class EmailSystem extends EmailSystemTemplate
{
  ...
  public function siteContato($cod) {
    ...
    if ($cod) {
      ...
      $body       = parent::emailHeader('InterAção', $html);
  ...
  public function siteCadastro($cod_cliente = null) {
    
    if ($cod_cliente) {
      
      $template = new Template( parent::getTemplate('site_cadastro.php') );
      ...
      $to_email   = $row['email'];
      $to_name    = $row['nome'];
      $subject    = 'Bem-vindo ao projeto InterAÇÃO';
      $body       = parent::emailHeader('Seu cadastro na InterAção', $html);
  ...
  public function enviaCompraPlanos($cod_cli_periodo) {
    ...
    if ($cod_cli_periodo) {
      ...
      $to_name    = 'InterAção';
      $subject    = 'Aquisição de novo plano na InterAção';
      $body       = parent::emailHeader('Aquisição de novo plano', $html);
  ...
  public function enviaAssinaturaManual($row, $html) {
    
    if ($row) {
      
      //$template = new Template( parent::getTemplate('assinatura_manual.php') );
      ...
      $to_email   = $row['email'];
      $to_name    = $row['nome'];
      $subject    = 'Finalização de Assinatura';
      $body       = parent::emailHeader('Finalização de Assinatura', $html);
