...
class Image {
  ...
  public function fit_width($width, $enlarge = false) {
    
    if ( $this->width > $width or ($enlarge and $this->width < $width)) {
        
      $nwidth = $width;
      $nheight = intval(($width / $this->width) * $this->height);
      
      $image = imagecreatetruecolor($nwidth, $nheight);
      imagealphablending($image, false);
      imagesavealpha($image, true);
      imagecopyresampled($image, $this->image, 0, 0, 0, 0, $nwidth, $nheight, $this->width, $this->height);
      $this->image = $image;
      $this->width = $nwidth;
      $this->height = $nheight;
  ...
  public function fit_height($height, $enlarge = false) {
    
    if ( $this->height > $height or ($enlarge and $this->height < $height)) {
        
      $nheight = $height;
      $nwidth = intval(($height / $this->height) * $this->width);
   ...
   * @acces public
   */
  function crop($x, $y, $width, $height, $w = null, $h = null) {
    ...
    if (!$w) $w = $width;
    if (!$h) $h = $height;
    imagecopyresampled($image, $this->image, 0, 0, $x, $y, $width, $height, $w, $h);
    $this->image = $image;
    $this->width = $width;
    $this->height = $height;
